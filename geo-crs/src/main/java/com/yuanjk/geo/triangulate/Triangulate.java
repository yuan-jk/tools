...
public class Triangulate {
    ...
    public void processResult(String[] args) {
        int nv = 20;
        if (args.length > 0 && args[0] != null)
            nv = new Integer(args[0]).intValue();
        if (nv <= 0 || nv > 1000)
            nv = 20;
    ...
    public static void main(String[] args) {
        ...
        try (FileWriter fw = new FileWriter("input_points.csv")) {
            for (int i = 0; i < points.length - 3; i++) {
                XYZ p = points[i];
                fw.write("Point(" + p.x + " " + p.y + ")" + "\n");
            }
        } catch (IOException e) {
        ...
        try (FileWriter fw = new FileWriter("output_triangles.csv")) {
            for (int i = 0; i < ntri; i++) {
                ITRIANGLE tri = triangles[i];
                fw.write("Polygon((" + points[tri.p1].x + " " + points[tri.p1].y + ", "
                        + points[tri.p2].x + " " + points[tri.p2].y + ", "
                        + points[tri.p3].x + " " + points[tri.p3].y + "))" + "\n");
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
